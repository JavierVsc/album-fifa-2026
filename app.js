...
function markVisibleAsOwned() {
  if (!confirm("¿Marcar todas las láminas faltantes del grupo visible como encontradas?")) return;
...
async function loadOfficialDataFromRepo() {
  ...
  try {
    const response = await fetch("album-data.txt?v=6", { cache: "no-store" });
...
function applyUiScale() {
  ...
  document.body.classList.toggle("view-compact", uiScale <= 0.92 && uiScale > 0.82);
